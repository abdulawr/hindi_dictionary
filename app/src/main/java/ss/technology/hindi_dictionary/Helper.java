...
public class Helper {
 ...
 private static Helper instanse;
    ...
    Cursor getQuestion(int id)
    {
        Cursor cursor=database.rawQuery("select english,hindi from word where _id = "+id+" or _id = "+(id+3)+" or _id = "+(id+6)+" or _id = "+(id+9)+";",null);
        return cursor;
    }

    Cursor getMeaning(String search)
    {
        String query="select * from word where english == upper("+"\'"+search+"\'"+");";
